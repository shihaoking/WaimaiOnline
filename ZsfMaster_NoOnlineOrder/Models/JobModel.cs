...
namespace ZsfProject.Models
{
    public class JobModel
    {
        ...
        public IQueryable<V_ShopJobAd> GetShopJobAds(int? shopId, int serviceAreaId, string userName,string jobName = "")
        {
            ...
            if(shopId != null)
                result = db.V_ShopJobAd.Where(r => r.ShopId == shopId.Value && r.JobTitle.Contains(jobName));
            else
            {
                if(!String.IsNullOrWhiteSpace(userName))
                {
                    ...
                    if(userInfo.GradeLevel >= 9)
                        result = db.V_ShopJobAd.Where(r => (r.PublishedByUserId == userInfo.Id || r.LastModifyByUserId == userInfo.Id) 
                            && r.JobTitle.Contains(jobName));
                    else
                        result = db.V_ShopJobAd.Where(r => r.AreaId == serviceAreaId && r.JobTitle.Contains(jobName));
                }
                else
                    result = db.V_ShopJobAd.Where(r => r.AreaId == serviceAreaId && r.JobTitle.Contains(jobName));
            }
            return result.Where(r=> r.JobTitle.Contains(jobName)).OrderByDescending(r => r.PublishedAt).ThenByDescending(r => r.OrderIndex);
        }
