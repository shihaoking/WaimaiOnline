...
$(function () {
    ...
    addMessage('欢迎来到外卖聊天室', 'notification');
    ...
    function InitName() {
        initNameTick++;
        var userNameReg = new RegExp("(^| )userName=([^;]*)(;|$)");
        var userName = document.cookie.match(userNameReg)[2];

        chat.server.send('/name ' + userName)
                .fail(function (e) {
                    console.error(e);
                    if (e == 'nameused' && initNameTick < 5)
                        InitName();
                }).done(function () { JoinRoom('ZSF'); });
